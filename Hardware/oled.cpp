/**
 * @file oled.c
 * @brief 波特律动OLED驱动(CH1116) - 标准库移植版
 * @anchor 波特律动(keysking 博哥在学习)
 * @version 1.1 (SPL Port)
 * @date 2023-08-04
 * @license MIT License
 *
 * @attention
 * 本驱动库已从HAL库移植到STM32标准外设库 (SPL).
 * 适用于使用标准库的STM32F1系列项目.
 *
 * @note
 * 使用流程:
 * 1. 在你的工程中初始化I2C (可参考 bsp_i2c.c).
 * 2. 调用OLED_Init()初始化OLED. 注意STM32启动比OLED上电快, 可等待20ms再初始化OLED.
 * 3. 调用OLED_NewFrame()开始绘制新的一帧.
 * 4. 调用OLED_DrawXXX()系列函数绘制图形到显存.
 * 5. 调用OLED_ShowFrame()将显存内容显示到OLED.
 */
#include "oled.h"
#include "bsp_i2c.h" // 引入I2C底层驱动
#include <math.h>
#include <stdlib.h>
#include "Delay.h"
...
/**
 * @brief 向OLED发送数据的函数 (标准库实现)
 * @param data 要发送的数据指针
 * @param len 要发送的数据长度
 * @return None
 * @note 此函数已从HAL库移植到标准库.
 */
...
void OLED_SendCmd(uint8_t cmd) {
  // 在标准库实现中, 我们需要构建一个2字节的数组
  // 第一个字节是控制字节 0x00 表示接下来是命令
  uint8_t sendBuffer[2];
  sendBuffer[0] = 0x00; // Co=0, D/C#=0, 表示后续为命令
  sendBuffer[1] = cmd;
...
void OLED_SendData(uint8_t data) {
    // 第一个字节是控制字节 0x40 表示接下来是数据
    uint8_t sendBuffer[2];
    sendBuffer[0] = 0x40; // Co=0, D/C#=1, 表示后续为数据
    sendBuffer[1] = data;
    OLED_Send(sendBuffer, 2);
}
 ...
 */
void OLED_ShowFrame() {
  // 在标准库实现中，我们需要将数据和控制字节分开
  // 先发送命令设置页和列地址，然后发送一整页的数据
  uint8_t sendBuffer[OLED_COLUMN + 1];
  sendBuffer[0] = 0x40; // 控制字节，表示接下来是数据
